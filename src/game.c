...
int newGame(Game *game, int nbPlayers){
	...
	game->players[0] = newPlayer(
			newBall(newPoint(0, 0), newVector(3, -4), 30, newColor(255, 0, 0)),
			newBar(newPoint(0, -300), 100, 20, newColor(255, 0, 0))
		);

	game->players[1] = newPlayer(
        	newBall(newPoint(0, 0), newVector(0, 0), 30, newColor(0, 255, 0)),
